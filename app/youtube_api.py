...
def get_youtube_channel_details(channel_tag: str) -> ChannelItem:
    ...
    logger.info(
        "get_youtube_channel_details: Fetching channel details for tag: %s", channel_tag)
    request = (
...
def get_youtube_playlist_details(channel_id: str) -> PlaylistResourceResponse:
    if youtube is None:
        raise Exception("Youtube API key not found")
    logger.info(
        "get_youtube_playlist_details: Fetching playlist details for channel: %s", channel_id)
    request = (
        youtube.playlists()
...
def get_videos_on_channel(
    ...
    if youtube is None:
        raise Exception("Youtube API key not found")
    logger.info(
        "get_videos_on_channel: Fetching videos on channel: %s", channel_id)
...
def get_all_videos_on_channel(channel_id: str) -> list[SearchResultItem]:
    if youtube is None:
        raise Exception("Youtube API key not found")
    logger.info(
        "get_all_videos_on_channel: Fetching all videos on channel: %s", channel_id)
    next_page_token: str | None = None
    all_videos: list[SearchResultItem] = []
    # This is equals to 2500 videos on a channel, which is most likely never the case
    max_requests = 50
    while max_requests > 0:
        ...
        logger.info(
            f"Found {len(video_result.items)} videos on page, next page token: {video_result.nextPageToken}, current page token: {next_page_token}")
...
def fetch_transcription(video_id: str) -> FetchedTranscript:
    if youtube is None:
        raise Exception("Youtube API key not found")
    logger.info(
        "fetch_transcription: trying to fetch transcription for video %s", video_id)
    try:
        ...
        try:
            ...
            logger.error(
                "Failed to fetch transcription for video %s , exception: %s", video_id, e)
...
def get_youtube_thumbnail_url(url: str, quality: str = "hqdefault") -> str:
    ...
    try:
        if not video_id:
            raise ValueError(
                "Failed to get thumbnail because video ID was not found in url")

        return f"https://img.youtube.com/vi/{video_id}/{quality}.jpg"
    except Exception as e:
        logger.error(f"Failed to get thumbnail for url: {url}, exception: {e}")
        raise ValueError(
            f"Failed to get thumbnail for url: {url}, exception: {e}")
...
def get_youtube_video_id_from_clip(url: str) -> str | None:
    ...
    """
    logger.info(
        "get_youtube_video_id_from_clip: Fetching video ID from clip url: %s", url)
    ...
    try:
        ...
        if hostname in ["youtu.be"]:
            ...
            if path.startswith("/watch"):
                ...
                video_id = path.split(
                    "/")[2] if len(path.split("/")) > 2 else None
