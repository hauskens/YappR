...
def video_chatlogs(video_id: int):
    ...
    logger.info("chat search: %s, user filter: %s", search, user_filter, extra={"video_id": video_id})
...
def video_category_chatlogs(video_id: int):
    ...
    logger.info("Getting category chatlogs for video", extra={"video_id": video_id})
    video = get_video(video_id)
    ...
    for category in categories:
        ...
        tag_conditions = [func.lower(ChatLog.message).contains(tag) for tag in lowercase_tags]
