    ...
    for metadata_file in metadata_files:
        ...
        try:
            ...
            if os.path.exists(result_file):
                ...
                metadata["completed_at"] = datetime.fromtimestamp(os.path.getmtime(result_file)).isoformat()
...
def download_transcription_srt(job_id):
    ...
    if os.path.exists(metadata_file):
        try:
            with open(metadata_file, "r") as f:
                metadata = json.load(f)
            original_name = os.path.splitext(metadata.get("original_filename", "transcription"))[0]
        except:
            pass
    ...
    try:
        ...
        logger.error(f"Error converting transcription to SRT: {e}", exc_info=True)
...
def upload_transcription_result(job_id):
    ...
    try:
        ...
        logger.error(f"Error uploading transcription result: {e}", exc_info=True)
        abort(500, description=f"Error uploading transcription result: {str(e)}")
...
def delete_transcription_job(job_id):
    ...
    for file_path in job_files:
        try:
            os.remove(file_path)
            logger.info(f"Deleted file {file_path}", extra={"user_id": current_user.id})
        except Exception as e:
            logger.error(f"Error deleting file {file_path}: {e}", extra={"user_id": current_user.id})
...
def serve_audio(job_id):
    try:
        ...
        if not audio_file:
            logger.error(f"Audio file not found for job {job_id} in {cache_dir}")
